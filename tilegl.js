...
(function () {
    ...
    TileRender.prototype = {
			...
			this._handler.addProgram(new og.Program("ndvi", {
				...
				"precision highp float;\n\
							...
							void main(){\n\
								...
								gl_FragColor = vec4(ndviColor.r, ndviColor.g, ndviColor.b, 1.0);\n\
							}"
			}));
		...
		render: function (srcImage) {
			...
			return this._framebuffer.readAllPixels();
		}
	};
					...
					ndviIDS.split(',').map(function(id) {
						...
						if (gmxLayer) {
							gmxLayer.setRasterHook(function (dstCanvas, srcImage, sx, sy, sw, sh, dx, dy, dw, dh, info) {
								if (srcImage) {
									var data = tileRender.render(srcImage);
